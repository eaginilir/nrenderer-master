...
namespace NRenderer
{
    ...
    void ScnImporter::computeTriangleVertexNormals(Asset& asset, size_t beginTri) {
        std::unordered_map<std::string, Vec3> normalSumMap;
        std::unordered_map<std::string, int> countMap;

        for (size_t i = beginTri; i < asset.triangles.size(); i++) {
            auto& tri = asset.triangles[i];
            Vec3 faceNormal = glm::normalize(tri->normal);

            for (auto v : {tri->v1, tri->v2, tri->v3}) {
                auto key = convert(v);
                normalSumMap[key] += faceNormal;
                countMap[key]++;
            }
        }

        auto getAvgNormal = [&](const Vec3& v) {
            std::string key = convert(v);
            return glm::normalize(normalSumMap[key] / static_cast<float>(countMap[key]));
        };

        for (size_t i = beginTri; i < asset.triangles.size(); ++i) {
            auto& tri = asset.triangles[i];
            tri->n1 = getAvgNormal(tri->v1);
            tri->n2 = getAvgNormal(tri->v2);
            tri->n3 = getAvgNormal(tri->v3);
        }

        std::cout << "[Info] Computed vertex normals for " 
                << (asset.triangles.size() - beginTri)
                << " triangles with " << normalSumMap.size() << " unique vertices.\n";
    }
    ...
    bool ScnImporter::import(Asset& asset, const string& path) {
        ...
        if (successFlag) {
            // 计算三角形的顶点法向量（用于Gouraud着色）
            if (asset.triangles.size() > beginTri) {
                computeTriangleVertexNormals(asset, beginTri);
            }
