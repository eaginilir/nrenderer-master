...
namespace SimplePathTracer::Intersection
{
    ...
    HitRecord xMesh(const Ray& ray, const Mesh& m, float tMin, float tMax) {
        ...
        for (size_t i = 0; i + 2 < m.positionIndices.size(); i += 3) {
            Index idx1 = m.positionIndices[i];
            Index idx2 = m.positionIndices[i + 1];
            Index idx3 = m.positionIndices[i + 2];
            if (idx1 >= m.positions.size() || idx2 >= m.positions.size() || idx3 >= m.positions.size()) {
                continue; // Skip invalid indices
            }
            Triangle tempTriangle;
            tempTriangle.v1 = m.positions[idx1];
            tempTriangle.v2 = m.positions[idx2];
            tempTriangle.v3 = m.positions[idx3];
            tempTriangle.normal = glm::normalize(glm::cross(tempTriangle.v2 - tempTriangle.v1, tempTriangle.v3 - tempTriangle.v1));
            auto hitRecord = xTriangle(ray, tempTriangle, tMin, closestT);
            if (hitRecord && hitRecord->t < closestT) {
                closestT = hitRecord->t;
                closestHit = hitRecord;
                closestHit->material = m.material; // Assign mesh material
            }
        }
        return closestHit;
    }
